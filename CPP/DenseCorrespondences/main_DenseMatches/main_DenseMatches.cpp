...
// Yuval additional includes
#include <DenseSIFT.h>
#include "Utilities.h"
#include "SIFT_FlowMatcher.hpp"
...
int main(int argc, char **argv)
{
	...
	std::string sSparseMatchesDir;
	std::string sOutDir = "";
	std::string sGeometricModel = "f";
	float fDistRatio = .6f;
	bool bScalemap = true;
	bool bTexturelessRegions = false;
	float dPeakThreshold = 0.04f;
	int sequentialLimit = 0;

	cmd.add(make_option('i', sImaDirectory, "imadir"));
	cmd.add(make_option('m', sSparseMatchesDir, "matchdir"));//
	cmd.add(make_option('o', sOutDir, "outdir"));
	cmd.add(make_option('r', fDistRatio, "distratio"));
	cmd.add(make_option('s', bScalemap, "scalemap"));//
	...
	cmd.add(make_option('t', bTexturelessRegions, "texturelessRegions"));//
	...
	denseSIFT.init(stepSize, bTexturelessRegions);
	denseSIFT.setOutputDir(sOutDir);
	double alpha = 2 * 255;
	double d = 40 * 255;
	...
	int topwsize = 20;
	int nIterations = 60;
	int nTopIterations = 100;
	denseSIFT.setSIFTFlowParams(d, nlevels, alpha, gamma,
		nIterations, nTopIterations, wsize, topwsize);
	...
	if (!bScalemap)
	{
		std::cout << "\n\nExtracting SIFT images" << std::endl;
		...
		{
			std::string sSIFTImg = stlplus::create_filespec(sOutDir,
				stlplus::basename_part(vec_fileNames[i]), "sift");

			//If descriptors or features file are missing, compute them
			if (!stlplus::file_exists(sSIFTImg))
			{
				//denseSIFT.extract(vec_fileNames[i], sSIFTImg, 3, stepSize, true);	// cellsize = 2
			}
			++my_progress_bar;
		}
	}
	...
	else // Compute the putatives matches
	{
		...
		SIFT_FlowMatcher collectionMatcher;

		if (collectionMatcher.loadData(vec_fileNames, vec_imagesSize, stepSize, sSparseMatchesDir, sOutDir, bScalemap))
		{
	...
	size_t iterations = 8192;	// 4096, 8192, 
	...
	// Print elapsed time
	cout << "Elapsed time = " << timer.elapsed().wall*1e-9f << " [s]" << endl;
