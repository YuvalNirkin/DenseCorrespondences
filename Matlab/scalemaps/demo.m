...
%source_name = 'smiley_src.png';dc
%target_name = 'smiley_dst.png';
source = imread(source_name);
target = imread(target_name);

%% Extract descriptors
tic
[desc_source, desc_target] = get_descriptors(source, target, 'match-aware', 'linear');
%[desc_source, desc_target] = get_descriptors(source, target, 'match-aware', 'exponential');
toc
%% SIFT-Flow
